...
public class FauxPropertiesUpdater {
	...
	private static final Log log = LogFactory.getLog(FauxPropertiesUpdater.class.getName());
	
	private static final String[] PATH_TO_PROPERTY_CONFIG = { "rdf", "display",
			"firsttime", "PropertyConfig.n3" };

	private static final String[] PATH_TO_PROPERTY_CONFIG_EXT = { "rdf", "display",
		"firsttime", "PropertyConfigExtension.n3" };
	...
	private boolean locateFileExt() {
		String homePath = ApplicationUtils.instance().getHomeDirectory()
				.getPath().toString();
		
		propertyConfigPath = Paths.get(homePath, PATH_TO_PROPERTY_CONFIG_EXT);
		if (Files.exists(propertyConfigPath)) {
			return true;
		} else {
			ss.warning(parent, "Could not find attributes "
