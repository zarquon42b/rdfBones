...
public class ImageUploadController extends FreemarkerHttpServlet {
	...
	public static final String ACTION_IND_UPLOAD = "individualUpload";
	...
	public static final String TEMPLATE_SHOW_UPLOADED = "imageUpload-showUploadedImages.ftl";
	...
	protected ResponseValues processRequest(VitroRequest vreq) {
		...
		try {
			...
			} else if (ACTION_IND_UPLOAD.equals(action)) {
				/**	New feature	**/
				return doIndividualUploadImage(vreq, entity);
			} else {
				captureReferringUrl(vreq);
	...
	private ResponseValues doIndividualUploadImage(VitroRequest vreq, Individual entity) {
		ImageUploadHelper helper = new ImageUploadHelper(fileStorage,
				vreq.getUnfilteredWebappDaoFactory(), getServletContext());

		try {
			// Did they provide a file to upload? If not, show an error.
			log.info("doIndividualUploadImage : entity \n" + entity);
	...
	private TemplateResponseValues showUploadedImages(VitroRequest vreq,
		...
		TemplateResponseValues rv = new TemplateResponseValues(TEMPLATE_SHOW_UPLOADED);
		...
		rv.put(BODY_FORM_ACTION, formAction(entity.getURI(), ACTION_IND_UPLOAD));
